	...
	return (
		<>
			{isLoading ? (
				...
				<>
					<div className={`w-[400px] relative border-2 border-gray-300 border-dashed rounded-lg p-6 ${dropzoneBorder}`} onDragEnter={(e) => handleDragEnter(e)} onDragOver={(e) => handleDragOver(e)} onDragLeave={(e) => handleDragLeave(e)} onDrop={(e) => handleDrop(e)}>
						<input className="absolute inset-0 w-full h-full opacity-0 z-50" type="file" onChange={(e) => handleFileSelect(e)} />
						<div className="text-center">
							...
							<p className="mt-1 text-xs text-gray-500">JSON up to 10MB</p>
						</div>
					</div>
					{data.fileList.length > 0 && <button onClick={uploadFiles}>Upload</button>}
				</>
			)}
		</>
	);
