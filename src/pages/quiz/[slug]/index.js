				...
				{isLoading ? (
					...
					<div className="h-[calc(100vh-130px)] max-w-6xl mt-[5rem] sm:mt-24 pb-5 mx-auto md:px-6 lg:px-8 bg-white md:bg-[#fcfcfc]">
						...
						<div className="flex flex-col justify-between items-center md:flex-row gap-6 h-full">
							<div className="bg-white md:bg-card-texture h-full bg-no-repeat bg-top p-5 ">
								<div class="max-w-4xl mx-auto grid grid-cols-1 lg:max-w-5xl lg:gap-x-20 lg:grid-cols-2">
									<div class="relative p-3 col-start-1 row-start-1 flex flex-col-reverse rounded-lg bg-gradient-to-t from-black/75 via-black/0 sm:bg-none sm:row-start-2 sm:p-0 lg:row-start-1">
										<h1 class="mt-1 text-lg font-semibold z-20 text-white sm:text-slate-900 md:text-2xl">{quiz.title}</h1>
										<p class="text-sm leading-4 font-medium z-20 text-white sm:text-slate-500">Quiz</p>
									</div>
									<div class="grid gap-4 col-start-1 col-end-3 row-start-1 sm:mb-6 sm:grid-cols-4 lg:gap-6 lg:col-start-2 lg:row-end-6 lg:row-span-6 lg:mb-0">
										<div class="relative w-full h-60 sm:h-52 sm:col-span-2 lg:col-span-full">
											<div class="absolute rounded-lg inset-0 overflow-hidden">
												<img src={quiz.image} alt={quiz.title} class="w-full h-full object-cover rounded-lg" loading="lazy" />
												<div class="rounded-lg absolute inset-0 bg-gradient-to-b from-transparent to-black opacity-50 sm:hidden"></div>
											</div>
										</div>
									</div>
									<div class="mt-4 text-xs font-medium flex items-center justify-between row-start-2 sm:mt-1 sm:row-start-3 md:mt-2.5 lg:row-start-2">
										<span>
											{quiz.rating != 0 && (
										...
										<span class="mx-3 text-slate-800 items-center flex gap-2">
									...
									<div class="mt-4 col-start-1 row-start-3 self-center sm:mt-0 sm:col-start-2 sm:row-start-2 sm:row-span-2 lg:mt-6 lg:col-start-1 lg:row-start-3 lg:row-end-4">
										<button onClick={() => router.push(quiz.slug + '/1')} type="button" class="text-white w-full sm:w-fit bg-sky-700 hover:bg-sky-800 focus:ring-1 focus:ring-sky-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-5 dark:bg-sky-500 dark:hover:bg-sky-600 focus:outline-none">
											Start
										</button>
									</div>
									<p class="mt-4 text-sm leading-6 col-start-1 sm:col-span-2 lg:mt-6 lg:row-start-4 lg:col-span-1 dark:text-slate-400">{quiz.description}</p>
								</div>
							</div>
						</div>
					</div>
